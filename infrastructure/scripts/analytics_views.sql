...
-- Run this script manually against the spendsmart_expense database AFTER
-- the expense-service Flyway migrations have run successfully.
--
-- Command:
--   docker exec -i infrastructure-postgres-1 psql -U postgres -d spendsmart_expense < analytics_views.sql
-- ============================================================================
...
-- Grant read access just in case analytics runs as a separate DB user
-- (No-op if analytics uses the same postgres user)
-- GRANT SELECT ON mv_daily_spend TO analytics_user;
-- GRANT SELECT ON mv_budget_utilization TO analytics_user;
